...
public class TypeTransformer {
    ...
    public static Expression T (Expression e, TypeMap tm) {
        ...
        if (e instanceof Unary) {
            Unary u = (Unary) e;
            Type tp = StaticTypeCheck.typeOf(u.term, tm);
            Expression t = T(u.term, tm);

            if ((tp == Type.BOOL) && (u.op.NotOp()))
                return new Unary(u.op.boolMap(u.op.val), t);
            else if ((tp == Type.FLOAT) && (u.op.NegateOp()))
                return new Unary(u.op.floatMap(u.op.val), t);
            else if ((tp == Type.INT) && (u.op.NegateOp()))
                return new Unary(u.op.intMap(u.op.val), t);
            else if((tp == Type.INT) && (u.op.charOp() || u.op.floatOp())) // int -> (char | float)
                return new Unary(u.op.intMap(u.op.val), t);
            else if((tp == Type.FLOAT) && (u.op.intOp()))   // float -> int
                return new Unary(u.op.floatMap(u.op.val), t);
            else if((tp == Type.CHAR) && (u.op.intOp()))    // char -> int
